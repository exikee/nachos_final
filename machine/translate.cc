...
#ifdef VMC
	ASSERT(PmEntry[ physicalAddress / 128 ] != NULL);
	PmEntry[ physicalAddress / 128 ]->use = true;
#endif
...
#ifdef VMC
	ASSERT(PmEntry[ physicalAddress / 128 ] != NULL);
	PmEntry[ physicalAddress / 128 ]->dirty = true;
#endif
    return TRUE;
}
...
// from the virtual address
    ...
    if (tlb == NULL) {		// => page table => vpn is index into table
		...
		for (entry = NULL, i = 0; i < TLBSize; i++)
    		if (tlb[i].valid && (((unsigned int)tlb[i].virtualPage) == currentThread->Space->pageTable[vpn].virtualPage)) {
				entry = &tlb[i];			// FOUND!
				break;
			}
