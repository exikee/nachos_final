...
#define DirectoryFileSize 	(10 * SectorSize )
#define VmFileSize 	(256 * SectorSize )
...
{ 
    ...
    if (format) {
		...
		FileHeader *vmHdr = new FileHeader;
    ...
    // (make sure no one else grabs these!)
		...
		freeMap->Mark(VmSector);
    // Second, allocate space for the data blocks containing the contents
    // of the directory and bitmap files.  There better be enough space!
		ASSERT(mapHdr->Allocate(freeMap, FreeMapFileSize));
		ASSERT(dirHdr->Allocate(freeMap, DirectoryFileSize));
		ASSERT(vmHdr->Allocate(freeMap, VmFileSize));
...
FileSystem::Create(char *name, int initialSize)
{
    ...
    if (directory->Find(name) != -1)
      success = FALSE;			// file is already in directory
		...
		DEBUG('f', "Add file: %s to Sector: %d\n", name, sector);
    	if (sector == -1) 		
	...
	DEBUG('f', "Add fileheader: %s to Sector: %d SUCCESS\n", name, sector);
    delete directory;
	sysLock->Release();
    return success;
}
...
bool FileSystem::MakeDir( char *name ){
	sysLock->Acquire();
	Directory *directory;
    BitMap *freeMap;
    bool success;
...
bool FileSystem::ChangeSize( FileHeader* hdr, int hdrSector, int newSize ){
	sysLock->Acquire();
    BitMap *freeMap;
    FileHeader *fileHdr = hdr;
    bool result;
	int sector;
    freeMap = new BitMap(NumSectors);
    freeMap->FetchFrom(freeMapFile);

    result = fileHdr->ChangeSize(freeMap,newSize);  		// remove data blocks
	if( result ){
		fileHdr->WriteBack(hdrSector);
		freeMap->WriteBack(freeMapFile);						// flush to disk
	}
    delete freeMap;
	sysLock->Release();
...
//fileopenlist
Lock* FileSystem::InsertFileEntry( int sector ){
	FileEntry* findFE;
...
//fileopen list
bool FileSystem::RemoveFileEntry( int sector ){
	FileEntry* find;
	map<int,FileEntry*>::iterator itFind;
	itFind = FileEntryList.find(sector);
	if( itFind != FileEntryList.end() ){
...
bool FileSystem::FileClose( OpenFile* file ){
	//delete file;
}
